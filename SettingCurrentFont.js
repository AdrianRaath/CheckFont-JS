...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const headingWeightDisplay = document.getElementById(
    "heading-weight-display"
  );
  ...
  function loadStoredFonts() {
    // A) Fonts
    selectedHeadingFont =
      localStorage.getItem("selectedHeadingFont") || "Inter";
    ...
    bodyLineHeight = parseFloat(
      localStorage.getItem("bodyLineHeight") || "1.5"
    );
    ...
    console.log("Loaded font size scales:", {
      headingSizeScale,
      bodySizeScale,
    });
    ...
    if (headingLetterSpacingInput) {
      ...
      if (headingLetterSpacingDisplay) {
        headingLetterSpacingDisplay.textContent =
          headingLetterSpacing.toFixed(2);
      }
    }
