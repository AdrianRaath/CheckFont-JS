...
document.querySelectorAll('[reset="font"]').forEach((element) => {
...
// --------------------------------------------------------------
document.querySelectorAll('[reset="color"]').forEach((element) => {
  element.addEventListener("click", () => {
    ...
    if (defaultColorRadio) {
      ...
      console.log(
        "Color reset to default popular color, custom inputs cleared."
      );
    }
  });
});
